...
async def create_product(payload: FinancialProductCreate, db: AsyncSession = Depends(get_db)):
    product = FinancialProduct(**payload.model_dump())
    db.add(product)
    await db.commit()
...
async def create_purchase(product_id: uuid.UUID, payload: PurchaseCreate, db: AsyncSession = Depends(get_db)):
    product = await db.get(FinancialProduct, product_id)
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
...
async def get_purchase_schedule(
    product_id: uuid.UUID, purchase_id: uuid.UUID, db: AsyncSession = Depends(get_db)
):
